...
export function Sidebar({ mobileOpen = false, onMobileClose }: SidebarProps) {
  ...
  const brand = (
    <div className="min-w-0">
      <p className="text-sm font-semibold tracking-tight text-wf-text">worshipflow2</p>
      <p className="text-[10px] font-medium uppercase tracking-[0.14em] text-wf-muted">Worship deck</p>
    </div>
  );

  const renderNavLinks = () => (
    <nav className="flex flex-1 flex-col gap-0.5 overflow-y-auto bg-wf-card/40 p-3 backdrop-blur-xl">
      {navItems.map((item) => {
        const active =
          item.href === "/dashboard"
            ? pathname === "/dashboard"
  ...
  const renderAccountBlock = () => (
    <div className="border-t border-wf-border bg-wf-card/40 p-3 backdrop-blur-xl">
      {!hydrated ? (
        <div className="h-[52px] animate-pulse rounded-[12px] bg-wf-text/[0.06]" aria-hidden />
      ) : session ? (
        <>
        ...
        <div className="flex flex-col gap-2">
          <Link
            href="/login"
            onClick={close}
            className="flex w-full items-center justify-center rounded-[12px] bg-blue-600 py-2.5 text-sm font-semibold text-white shadow-md shadow-black/30 transition hover:brightness-110"
          >
            Sign in
          </Link>
          <Link
  ...
  const panelInner = (
    <>
      <div className="flex h-14 shrink-0 items-center justify-between border-b border-wf-border bg-wf-card px-4">
        {brand}
        <button
          type="button"
          onClick={close}
          className="flex h-10 w-10 items-center justify-center rounded-xl text-lg text-wf-muted transition hover:bg-white/[0.06] hover:text-wf-text md:hidden"
  ...
  return (
    <>
      {/* Desktop sidebar */}
      <aside className="hidden w-[240px] shrink-0 flex-col border-r border-wf-border md:flex">
        <div className="flex h-14 items-center border-b border-wf-border bg-wf-card px-5">{brand}</div>
        {renderNavLinks()}
        {renderAccountBlock()}
      </aside>
      ...
      {mobileOpen ? (
        <>
          ...
          />
          <aside
            className="fixed inset-y-0 left-0 z-50 flex w-[min(100vw-3rem,18rem)] max-w-[300px] flex-col border-r border-wf-border bg-wf-bg shadow-2xl md:hidden"
            role="dialog"
            aria-modal="true"
