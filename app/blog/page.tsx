    ...
    <>
      <BlogShell>
        ...
        <p className="mt-3 text-sm leading-relaxed text-wf-muted">
          ...
          our free{" "}
          <Link href="/#lyric-splitter" className="font-medium text-violet-300/90 hover:underline">
        ...
        <ul className="mt-10 space-y-6">
          {BLOG_POSTS.map((p) => (
            <li key={p.slug}>
              <article className="rounded-2xl border border-wf-border bg-wf-card/40 p-5 shadow-lg shadow-black/15 backdrop-blur-sm transition hover:border-violet-500/25">
                <time className="text-[11px] font-medium uppercase tracking-wider text-wf-muted" dateTime={p.date}>
                  {p.date}
                </time>
                <h2 className="mt-2 text-xl font-semibold tracking-tight">
                  <Link href={`/blog/${p.slug}`} className="text-wf-text hover:text-violet-200">
                    {p.title}
                  </Link>
                </h2>
                ...
                <Link
                  href={`/blog/${p.slug}`}
                  className="mt-3 inline-block text-sm font-medium text-violet-300/90 hover:underline"
                >
                  Read more →
                </Link>
              </article>
